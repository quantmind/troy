...
None of the code here is hand-written. Every line, along with the tests, the
benchmarks and the documentation, is generated by an AI agent working under the
close supervision of high frequency trading practitioners, who set the design
constraints, review each change and decide what ships.

Issues and pull requests are welcome on the same terms: describe the behaviour
you want and why it matters on a hot path, and expect the change itself to come
back generated and reviewed rather than typed. A change lands only with `make
rs-lint` and `make rs-test` clean, and anything touching the arithmetic also has
to survive `make rs-oracle` and `make rs-fuzz`, the 256-bit reference and the
property sweeps that stand in for a human reading every line.

Speed is a correctness condition here, so a change also has to leave the
benchmarks where it found them. Run `make bench` before and after on the same
idle machine and compare against `make bench-table`, criterion reports the
change per operation. A regression on a hot path needs a reason worth the
nanoseconds, and if the numbers improve, `make bench-save` on the reference
machine records the new snapshot the published page is rendered from.
